...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      cartItems: [
        ...
        {
          "id": 8,
          "title": "Pierced Owl Rose Gold Plated Stainless Steel Double", "price": 10.99,
        ...
        {
          "id": 9,
          "title": "WD 2TB Elements Portable External Hard Drive - USB 3.0 ", "price": 64,
  ...
  render() {
    ...
    return (
      <div className="App">
        { cartItems.map(item => (
          <Container style={{border:'2px solid black', margin:'10px'}}>
            <img src={item.image} alt="product" height="200"/>
            <h1>{item.title}</h1>
            <h2>{item.price}</h2>
            <Button variant="dark" style={{margin:'5px'}} onClick={this.remove}>Remove Product</Button>
          </Container> 
        ))}   
      </div>
